...
'variance_weighted'] or None or array-like of shape (n_outputs)
    ...
    numerator = (weight * (y_true - y_pred) ** 2).sum(axis=0,
                                                      dtype=torch.float64)
    # REVIEW Check this out
    denominator = (weight * (y_true - pm.average(
        y_true, axis=0, weights=sample_weight)) ** 2).sum(axis=0,
                                                          dtype=torch.float64)
    nonzero_denominator = denominator != 0
    nonzero_numerator = numerator != 0
    valid_score = nonzero_denominator & nonzero_numerator
    output_scores = torch.ones([y_true.shape[1]], dtype=torch.float64)
    output_scores[valid_score] = 1 - (numerator[valid_score] /
                                      denominator[valid_score])
    ...
    if isinstance(multioutput, str):
        if multioutput == 'raw_values':
            ...
            if not torch.any(nonzero_denominator):
                if not torch.any(nonzero_numerator):
                    return 1.0
                else:
                    return 0.0
    else:
    ...
    return pm.average(output_scores, weights=avg_weights)
